          ...
          if(!StrUtils.isBlank(uUserLogin)) {
              ...
              if(StrUtils.isBlank(request.getSession().getAttribute("surveyor"))){
                  if(!StrUtils.isBlank(uUserLogin.getUsrRc0())&&!StrUtils.isBlank(uUserLogin.getUsrRc1())){
                      Surveyor sur = surveyorService.get(uUserLogin.getUsrRc0());//验船师基本信息
                      OrganInfo org=organService.get(uUserLogin.getUsrRc1());//机构基本信息
                      request.getSession().setAttribute("surveyor",sur);
                      request.getSession().setAttribute("orginfo",org);
                  }
              }
          }
      }catch (Exception e){
