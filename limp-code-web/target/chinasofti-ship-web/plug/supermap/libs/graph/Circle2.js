...
SuperMap.Feature.Theme.Circle = SuperMap.Class(SuperMap.Feature.Theme.RankSymbol, {
    ...
    assembleShapes: function(){
		...
		var bestPro = sets.bestProvince;
		var bestColor = sets.bestColor;
		
		var normalPro = sets.normalProvince;
		var normalColor = sets.normalColor;
		
		var badPro = sets.badProvince;
		var badColor = sets.badColor;
...
//          //用户自定义
//         if(name == '河北省' || name == '天津市' || name == '福建省' || name == '广东省' || name=='广西壮族自治区' || name=='海南省' || name=='湖北省' || name=='湖南省'){
//          	circleSP.style.fillColor = '#abb7c4';
//          }else if(name == '安徽省' || name == '吉林省' || name == '黑龙江省' || name =='四川省' || name == '江西省' || name == '贵州省' || name=='重庆市' || name == '河南省' || name == '云南省' || name == '甘肃省' ){
//          	circleSP.style.fillColor = '#CC6699';
//          }else{
...
//      }
        ...
        if(typeof (sets.fillColor) == "undefined" && bestPro.length != 0 ){
        	for(var i=0;i<bestPro.length;i++){
        		if(name == bestPro[i]){
        			circleSP.style.fillColor = bestColor[0];
        		}
        	}	
        }else{
        	//circleSP.style.fillColor = '#009900'
        }
        
        if(typeof (sets.fillColor) == "undefined" && normalPro.length != 0 ){
        	for(var i=0;i<normalPro.length;i++){
        		if(name == normalPro[i]){
        			circleSP.style.fillColor = normalColor[0];
        		}
        	}	
        }else{
        	//circleSP.style.fillColor = '#CCCC00'
        }
        
        if(typeof (sets.fillColor) == "undefined" && badPro.length != 0 ){
        	for(var i=0;i<badPro.length;i++){
        		if(name == badPro[i]){
        			circleSP.style.fillColor = badColor[0];
        		}
        	}	
        }else{
        	//circleSP.style.fillColor = '#FF3300'
        }
