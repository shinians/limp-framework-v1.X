...
(function($){
    ...
    $.fn.LPshowLoading=function(type){
        var loadId=$(this).attr("id");//loadingID

        //type为空默认的加载在做左上角
        if(system.isBlank(type)){
            var loadDiv=$("<div style='width: 150px;height: 100px' id='loadDiv'></div>");
            $("#"+loadId).html(loadDiv);
            loadId="loadDiv";
        }else if("1"==type){
            $("#loadDiv").remove();
            var loadDiv=$("<div style='width: 131px;height: 80px;margin-left: -41px;margin-top: -25px' id='loadDiv'></div>");
            $("#"+loadId).append(loadDiv);
            loadId="loadDiv";
        }
        var sysLoadChart = echarts.init(document.getElementById(loadId));
        sysLoadChart.showLoading('default', {text:'loading...',maskColor: 'transparent',textColor: '#c23531'}) ;
    };
    ...
    $.fn.LPTable=function(action,paramData,options){
        ...
        if(!system.isBlank(action)){
            system.post(action,paramData,function(data){
                ...
                //--------------------------分页列表-------------------------------------/
                var divPage=$('<div style="text-align: center" class="page-div"></div>');
                var pageList=$('<ul class="pagination" id="'+pageId+'"></ul>');
                ...
                if(parseInt(page)>1){
                    pageList.append('<li><a href="javascript:void(0)" style="color: #ccc;cursor: default;">共'+system.dealBlank(data.result.rowCount,0)+'条数据</a></li>');
                    pageList.append('<li><a href="javascript:void(0)" title="第1页" data-value="1">首页</a></li>'); //第一页
                    pageList.append('<li><a href="javascript:void(0)" title="" data-value="'+(parseInt(page)-1)+'">上一页</a></li>'); //第一页
                }else{
                    pageList.append('<li><a href="javascript:void(0)" style="color: #ccc;cursor: default;">共'+system.dealBlank(data.result.rowCount,0)+'条数据</a></li>');
                    pageList.append('<li><a href="javascript:void(0)" title="第1页" style="color: #ccc;cursor: default;">首页</a></li>'); //第一页
                    pageList.append('<li><a href="javascript:void(0)" title="" style="color: #ccc;cursor: default;" >上一页</a></li>'); //第一页
                }
                ...
                if(parseInt(page)<system.PagesLen){
                    pageList.append('<li><a href="javascript:void(0)" title="下一页" data-value="' + (parseInt(page)+1) + '">下一页</a></li>');
                    pageList.append('<li><a href="javascript:void(0)" title="第' + system.PagesLen + '页" data-value="' + system.PagesLen + '">尾页</a></li>');
                }else{
                    pageList.append('<li><a href="javascript:void(0)" title="下一页" style="color: #ccc;cursor: default;" >下一页</a></li>');
                    pageList.append('<li><a href="javascript:void(0)"  style="color: #ccc;cursor: default;" title="第' + system.PagesLen + '页" >尾页</a></li>');
                ...
                $("#"+pageId+ " li").click(function () {
                    var pageCurr=$(this).find("a").attr("data-value");
                ...
                $("#"+lpId+" th").click(function(){
                    if($(this).find("span").hasClass("lpsort")){
                        if($(this).find("span").hasClass("lphide")){
                            //所有的都制成隐藏状态
                            $(".lpsort").each(function(){
